 ...
 */
export function expose<T extends object, Keys extends Array<keyof T>>(
  obj: T,
  ...keys: Keys
) {
  ...
  return xprt as Pick<T, typeof keys[number]>;
}

export const pick = expose;

/**
 * Create a new object without listed property keys
 */
export function omit<T extends object, Keys extends Array<keyof T>>(
  obj: T,
  ...keys: Keys
) {
  const xprt: Partial<T> = {};
  ...
  return xprt as Omit<T, typeof keys[number]>;
}
